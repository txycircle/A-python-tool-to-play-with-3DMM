...
def main():
    ...
    modelconfig.image_path = os.path.join(modelconfig.image_path, 'uv')

    if not os.path.isdir(modelconfig.image_path):
        os.makedirs(modelconfig.image_path)

    ######UV#########
    uv_map = np.array( sio.loadmat(modelconfig.uvmap_path)['UV_map'])*modelconfig.imagesize-1
    uv_map[:,1] = modelconfig.imagesize*1.0-uv_map[:,1]
    ...
    ######Zbuffer uv#######
    tranfer_uv = Zbuffer_uv(uv_map, Tri, modelconfig.imagesize)
    barycentric_coordinates_uv, triangle_ids_uv = tranfer_uv.forward()
    ...
    ######render uv color#######
    rander_uv = Render_color(barycentric_coordinates_uv, triangle_ids_uv, texture, Tri, modelconfig.imagesize)
    uv_map_image = rander_uv.forward()

    img = Image.fromarray(np.uint8(image))
    img_uv = Image.fromarray(np.uint8(uv_map_image))
    img.show()
    img_uv.show()
    image_path = os.path.join(modelconfig.image_path,  'A.jpg')
    image_uv_path = os.path.join(modelconfig.image_path,  'A_uv.jpg')
    img.save(image_path)
    img_uv.save(image_uv_path)
